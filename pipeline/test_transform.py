...
def test_successful_load_data(sample_csv_data):
    """Test csv data is loaded into df successfully."""
    df = load_data(sample_csv_data)
    assert df.shape == (1, 9)
    assert "last_watered" in df


def test_recording_taken_data_type(sample_pd):
...
def test_email_valid(sample_pd):
...
def test_null_value_discard_row():
